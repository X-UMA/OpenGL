...
* RenderingPart	を作成する
*
* @param size	描画するインデックス
* @param offset	描画開始インデックスのオフセット(インデックス単位)
*
* @return 作成した部分描画オブジェクト
*/
constexpr RenderingPart MakeRendringPart(GLsizei size, GLsizei offset) {
...
static const RenderingPart renderingParts[] = {
	MakeRendringPart(12,0),
	MakeRendringPart(6,12),
};
...
* @param tilte	ウィンドウタイトル(UTF-8の0終端文字列)
*
* @retval true	初期化成功
* @retval false	初期化失敗
*
* Run 関数を呼び出す前に、一度だけ呼び出しておく必要がある、
* 一度初期化に成功すると、以降の呼び出しでは何もせずにtrueを返す
*/
bool GameEngine::Init(int w, int h, const char* title) {
	...
	meshBuffer = Mesh::Buffer::Create(10 * 1024 * 2, 30 * 1024 * 2);
	if (!meshBuffer) {
		std::cerr << "ERROR: GameEngineの初期化に失敗" << std::endl;
		return false;
	}
	entityBuffer = Entity::Buffer::Create(
		1024, sizeof(Uniform::VertexData), 0, "VertextData");
	if (!entityBuffer) {
		std::cerr << "ERROR: GameEngineの初期化に失敗" << std::endl;
		return false;
	}
...
bool GameEngine::LoadTextureFromFile(const char* filename) {
	...
	TexurePtr texture = Texture::LoadFromFile(filename);
	if (!texture) {
		return false;
	}
...
Entity::Entity* GameEngine::AddEntity(int groupId, const glm::vec3& pos, const char* meshName,
	...
	const TexurePtr& tex = textureBuffer.find(texName)->second;
...
*
* 衝突が発生し衝突解決ハンドラが呼び出される時、より小さいグループIDを持つエンティティから先に渡される
* ここで指定したグループIDの順序とは無関係であることに注意すること
*ex)
*	CollisionHandler(10，1，Func)
*	というコードでハンドラを登録したとする。衝突が発生すると、
*	Func(グループ ID=1のエンティティ、グループ ID=10のエンティティ
*	のように呼び出される。
*/
...
void GameEngine::Render() const {
	...
	progColorFilter->BindTexture(GL_TEXTURE0, GL_TEXTURE_2D, offscreen->GetTextre());
	//progColorFilter->BindTexture(GL_TEXTURE0, GL_TEXTURE_2D, textureBuffer.find("Res/superman.bmp")->second->Id());

...
namespace Shader {
	...
	* シェーダープログラムを作成する
	* 
	* @param vsCode		頂点シェーダーへのポインタ
	* @param fsCode		フラグメントシェーダーへのポインタ
	*
	...
	*/
	void Program::UseProgram() {
		...
		for (GLint i = 0; i < samplerConut; ++i) {
	...
	*/
	void Program::BindTexture(GLenum unit, GLenum type, GLuint texture) {
		if (unit >= GL_TEXTURE0 && unit < static_cast<GLenum>(GL_TEXTURE0 + samplerConut)) {

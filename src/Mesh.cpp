...
namespace Mesh {
	...
	*/
	bool FbxLoader::LoadMesh(FbxNode* fbxNode) {
		...
		//マテリアル情報を読み取る
		const int materialConut = fbxNode->GetMaterialCount();
		mesh.materialList.reserve(materialConut);
		for (int i = 0; i < materialConut; ++i) {
		...
		//ポリゴン数に基づいて、仮データバッファの容量を要約する
		const int polygonConut = fbxMesh->GetPolygonCount();
		for (auto& e : mesh.materialList) {
			const size_t avarageCapacity = polygonConut / mesh.materialList.size();
		...
		int polygonVertex = 0;
		for (int polygonIndex = 0; polygonIndex < polygonConut; ++polygonIndex) {
